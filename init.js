...
var singleModule = (function(){
    ...
    var validModules = ['ETHWorker', 'TLOWorker'];
...
function spawnETHWorker(){
    if (!config.ethereum || !config.ethereum.enabled || !config.ethereum.ownerKey || !config.ethereum.contractAddress) return;

    var ETHWorkers = {};

    var createETHWorker = function(forkId){
        var worker = cluster.fork({
            workerType: 'ETHWorker',
            forkId: forkId
        });
        worker.forkID = forkId;
        worker.type = 'ETHWorker';
        worker.on('exit', function(code, signal){
            log('error', logSystem, 'ETH worker died, spawning replacement worker...');
            setTimeout(function(){
                createETHWorker(forkId);
            }, 2000);
        });
    };

    createETHWorker('1');
    log('info', logSystem, 'Spawned ETH worker');
}
