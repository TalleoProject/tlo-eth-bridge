...
function confirmEtherTransaction(txHash) {
    ...
    setTimeout(async function () {
        ...
        if (txConfirmations >= (global.config.ethereum.confirmations || 10)) {
            ...
            global.redisClient.hget('tlo-base-bridge:ethereum', 'conversion' + txHash, function (err, result) {
                ...
                if (result === null && etherTransfer._value > 100) {
                    ...
                    apiInterfaces.rpcWallet('sendTransaction', transferRPC, function (err1, response1) {
                        ...
                        if (TLOHash) {
                            ...
                            global.redisClient.hset('tlo-base-bridge:ethereum', 'conversion' + txHash, TLOHash, function (err2) {
                                if (err2) {
...
(function init() {
    "use strict";
    global.redisClient.hget('tlo-base-bridge:ethereum', 'lastBlockFound', function (error, result) {
        if (error) {
            lastBlockFound = 0;
        } else {
        ...
        fs.readFile(global.config.ethereum.contractABI, { encoding: 'utf-8' }, function (error1, data) {
            ...
            tokenContract.events.ConversionTo(options, function (error2, event) {
               ...
               if (event.blockNumber && event.blockNumber > lastBlockFound) {
                   global.redisClient.hset('tlo-base-bridge:ethereum', 'lastBlockFound', event.blockNumber, function (error3) {
                       if (error3) {
