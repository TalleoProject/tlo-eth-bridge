            ...
            apiInterfaces.rpcWallet('getTransaction', {'transactionHash': txHash}, function (error, result) {
                ...
                if (result.transaction.blockIndex > 0) {
                    if (result.transaction.blockIndex > lastBlockFound) {
                        global.redisClient.hset('tlo-eth-bridge:talleo', 'lastBlockFound', result.transaction.blockIndex, function (error1) {
                            if (error1) {
...
function confirmTalleoTransaction(txHash) {
    ...
    setTimeout(async function() {
        ...
        if (txConfirmations >= (global.config.talleo.confirmations || 10)) {
            ...
            global.redisClient.hget('tlo-eth-bridge:talleo', 'conversion' + txHash, function (error, result) {
                                ...
                                nonce => {
                                    ...
                                    web3.eth.accounts.signTransaction(
                                    {
                                        ...
                                        web3.eth.sendSignedTransaction(result4.rawTransaction, function (error5, result5) {
                                            ...
                                            global.redisClient.hset('tlo-eth-bridge:talleo', 'conversion' + txHash, hash, function (error7, result7) {
...
(function init() {
    "use strict";
    global.redisClient.hget('tlo-eth-bridge:talleo', 'lastBlockFound', function (error, result) {
        if (error) {
            lastBlockFound = 0;
        } else {
